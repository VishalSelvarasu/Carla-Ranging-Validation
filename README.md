...
The source video is generated by `src/make_video.py` as `results/comparison.avi`; the PNG and GIF are the repository-host-friendly views.
...
- `results/fig1_range_error_vs_severity.png` — range-error figure.
- `results/comparison.avi` — source qualitative comparison video.
- `results/comparison_frame.png` — static GitHub-renderable comparison frame.
- `results/comparison.gif` — GitHub-renderable qualitative comparison excerpt.
- `dataset/`, `dataset_s*/` — generated capture data; gitignored.
- `results/`, `results_s*/`, `results_pooled/` — generated analysis outputs; seed-specific/pooled directories are gitignored as configured by the repo.

# Validation engineering: claims the data rejected

The strongest part of this benchmark is not a single effect size; it is the sequence of plausible claims that were tested and removed when the seed-level data did not support them.

| Hypothesis | Validation outcome |
| --- | --- |
| **Z² crossover** | Not supported by the measured data; removed rather than rationalized after the fact. |
...
| **Closing-speed filter explains/strengthens the effect** | Not supported by the command-level check; the filter was not retained as part of the result. |
